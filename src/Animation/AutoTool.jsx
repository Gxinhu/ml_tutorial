...
class SubTool extends Component {
  ...
  linearRegression() {
    tf.tidy(() => {
      ...
      function loss(y, y_pred) {
        return (
          (1 / (2 * len)) * tf.sum(tf.pow(tf.sub(y_pred, y), 2)).dataSync()[0]
        );
      }
      ...
      const dlossdb = (1 / len) * tf.sum(tf.sub(yPred, yTensor)).dataSync()[0];
  ...
  render() {
    ...
    const gap = 50;
    const totalWidth = plotWidth + panelWidth + gap;
    return (
      <div
        style={{
          width: '100%',
      ...
      >
        ...
        >
          ...
          <div style={{ width: `${panelWidth}px`, flex: `0 0 ${panelWidth}px` }}>
            <Card variant="outlined">
              <CardContent>
                <Typography variant="h4" component="h2" style={{ fontSize: 20 }}>
                  EPOCH: {epochs}
                </Typography>
                <Typography variant="h6" style={{ fontSize: 18, lineHeight: 1.45 }}>
                  {$('\\text{neuron}(x) = ')}
                  <span
                    style={{
                      color: blue,
                ...
                <IconButton onClick={this.reset} size="small">
                  <Replay />
                </IconButton>
                <Fab
                  style={{
                    background: this.state.playButton ? '#175676' : '#D62839',
                    color: 'white',
                    width: 42,
                    height: 42,
                    minHeight: 42,
                  ...
                  disableTouchRipple
                >
                  {this.state.playButton ? <PlayArrow /> : <Stop />}
                </Fab>
