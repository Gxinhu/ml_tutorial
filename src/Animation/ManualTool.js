      ...
      <Box display="flex" justifyContent="center">
        ...
        <Box marginLeft={7}>
          <Card variant="outlined">
            <CardContent>
              <p style={{ fontSize: 20 }}>
                Manually tune <em style={{ color: weightColor }}>weight</em> and{' '}
                <em style={{ color: biasColor }}>bias</em> and try to reach a{' '}
              ...
              <Slider
                ...
                value={weight}
                onChange={(e, n) => {
                  const newLine = this.computeLine(weight, bias, domain);
                  const loss = this.loss(range, newLine, range.length);
                  this.setState({
                    weight: n,
                    line: newLine,
                    loss,
              ...
              <p style={{ fontSize: 20 }}>
                ...
                {$('x + ')} <em style={{ color: biasColor }}>{$(`${bias}`)}</em>
              </p>
              <p style={{ fontSize: 20 }}>
                <i style={{ color: lossColor }}>loss</i>{' '}
                {$(` = \\frac{1}{J}\\sum_{i = 1}^J (\\hat{y_i} - y_i)^2 =`)}
                <em style={{ color: lossColor }}> {$(`${loss.toFixed(7)}`)}</em>
              </p>
