    ...
    <svg id="p" width="810" height="500" overflow="visible">
      <g id="ee">
        <path
          ...
          className={mode && keyFrameLoss > 0 ? keyFrameLossCalc() : playing}
        ></path>
        ...
        {shapedLinks.map((layer, i) => {
          return layer.map((neuron, j) => {
            ...
            if (mode && subEpoch === 'forward' && i === keyFrameLayer) {
              moving = 'edgeForward';
              colorChange = true;
            } else if (mode && subEpoch === 'backward' && i === keyFrameLayer) {
              moving = 'edgeBackward';
              colorChange = true;
            }
            return neuron.map((d, k) => {
              /* handle undefined outputs during re render */
              const isUndefined = isUndefined3d(shapedWeights, i, j, k);
              const currentWeight = isUndefined ? [] : shapedWeights[i][j][k];
              const sw = isUndefined ? 1 : 2 * Math.abs(currentWeight) + 0.1;
              const s = isUndefined
                ? 'lightgrey'
                : currentWeight > 0
                ? posWeight
              ...
              if (mode && subEpoch === 'update' && isAnimating === true) {
                let gradient = 2 * miniNN.model[i][j].dWeights[k];
        ...
        {shapedRects.map((neuron, i) =>
          neuron.map((d, j) => {
            ...
            const beforeUpdate =
              subEpoch === 'backward' || subEpoch === 'transition';
            if (
              ...
              <g key={`${i},${j}`}>
                  ...
                  fill={
                    d.x === 734 && d.y === 234 && mode
                      ...
                      : d3
                          .rgb(104, 104, 104)
                          .brighter(
                            actColor ? miniNN.model[i][j].actStep + lr : 1,
                          )
                  }
                  stroke={d.x === 734 && d.y === 234 ? '#507BB6' : 'black'}
                  strokeWidth={3}
                ></rect>
                ...
                miniNN.model[i][j].dActStep !== 0
                  ? VerticalArrow(
                      d.x + 16,
                      d.y + 16,
                      Math.abs(miniNN.model[i][j].dActStep),
                      miniNN.model[i][j].dActStep < 0,
                      colorOfMotion,
                    )
                  : ''}
